...
//
GLboolean ESUTIL_API esCreateWindowAndroid(ESContext *esContext, const char *title)//, GLint width, GLint height, GLuint flags )
{
    EGLConfig config;
    EGLint majorVersion;
    EGLint minorVersion;
    ...
    esContext->eglDisplay = eglGetDisplay(esContext->eglNativeDisplay);//EGL_DEFAULT_DISPLAY);
    // EGLDisplay display = eglGetDisplay(EGL_DEFAULT_DISPLAY);
    if (esContext->eglDisplay == EGL_NO_DISPLAY) {
        esLogMessage("eglGetDisplay Failed");
        return GL_FALSE;
    }

    if (!eglInitialize(esContext->eglDisplay, &majorVersion, &minorVersion)) {
        esLogMessage("eglInitialize Failed");
        return GL_FALSE;
    } 
    ...
    if (!eglChooseConfig(esContext->eglDisplay, attribList, &config, 1, &numConfigs)) {
        esLogMessage("eglChooseConfig Failed");
        return GL_FALSE;
    }

    if (numConfigs < 1) {
        esLogMessage("numConfigs < 1");
        return GL_FALSE;
    }
...
//    surface = eglCreateWindowSurface(display, config, s.get(), NULL);
    if (esContext->eglSurface == EGL_NO_SURFACE) {
        esLogMessage("eglCreateWindowSurface Failed");
        return GL_FALSE;
    }
    ...
    if (esContext->eglContext == EGL_NO_CONTEXT) {
        esLogMessage("eglCreateContext Failed");
        return GL_FALSE;
    }
    ...
    if (!eglMakeCurrent(esContext->eglDisplay, esContext->eglSurface,
        ...
        esLogMessage("eglMakeCurrent Failed");
        return GL_FALSE;
    }
